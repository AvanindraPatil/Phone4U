...
if (!empty($_POST)) {
    ...
    // if(in_array(""))
    // die(print_r($_POST));
    // dd(in_array("5G", array_values($_POST)));
    $keys = array_values($_POST);
    $firstPreference = $keys[0];
    // dd($firstPreference);
    $secondPreference = $keys[1];
    $thirdPreference = $keys[2];
    // dd(print_r($keys));
    // $first_preference = $_POST['first_preference'];
    
    foreach ($_POST as $key => $value) {
        if (strcasecmp($key, "budget")!=0 && str_word_count($value) > 1) {
            $_POST[$key] = str_replace(" ", "_", strtolower(trim($value)));
        } else {
    ...
    $budget = explode("-", str_replace("₹", "", $_POST['budget']));
    // dd($lowerThanBudget);
    // dd(print_r($budget));
    $startPrice = $budget[0];
    $endPrice = $budget[1];
    ...
    // die($_POST['first_preference']);
    if (in_array("5g", array_values($_POST))) {
        // dd("TRUE");
        // die();
    }
    ...
    if (isset($_POST['lowerThanBudget'])) {
        ...
        $query = "SELECT * FROM phone JOIN score on phone.phone_id = score.phone_id WHERE $first_preference >= 8 and $second_preference >= 6 and $third_preference >= 4  and brand_id = $brandId and phone_price < $endPrice  ORDER BY average DESC";
    } else {
...
<body data-spy="scroll" data-target=".fixed-top" id="result">
    ...
    <nav class="navbar navbar-expand-md navbar-dark navbar-custom fixed-top">
        ...
        <div class="collapse navbar-collapse" id="navbarsExampleDefault">
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link page-scroll" href="index.html?#news">TECHQUICKIE</a>
                </li>

                <!-- Dropdown Menu -->
                <li class="nav-item dropdown">
                    <a class="nav-link  page-scroll" href="index.html?#about" id="navbarDropdown" role="button" aria-haspopup="true" aria-expanded="false">ABOUT</a>
                </li>
                <!-- end of dropdown menu -->
                <li class="nav-item">
                    <a class="nav-link page-scroll" href="index.html?#form">GET STRARTED<span class="sr-only">(current)</span></a>
                </li>
            </ul>
    ...
    <div class="copyright">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <p class="p-small">Copyright ©️ 2021 <a href="http://gpmumbai.ac.in" style="text-decoration: underline;">Phone4U</a></p>
                </div> <!-- end of col -->
            </div> <!-- enf of row -->
        </div> <!-- end of container -->
